/*
Date:16 May 2018
This tool is to generate data for building transcript cluster from simulated data from Polyester
Note:This implementation is adapted from SailfishQuantify.cpp of Sailfish for our purpose. 

Date:12 Mar 2021
maxReadOcc=10000000 as the default setting
*/
...
//  SACollector<IndexT> hitCollector(sidx);
  SACollectorMAX<IndexT> hitCollector(sidx);
...
*/
        ...
        if (jointHits.size() > 0) {
            //MAX - extract txname from read's header
            //std::cout << " \n header " << j->data[i].first.header<<std::endl;
            std::string delimiter1 = "/";
            std::string delimiter2 = " ";
            std::string readTxName;
            std::string::size_type pos1 = 0;
            std::string::size_type pos2 = 0;
            pos1 = j->data[i].first.header.find(delimiter1);
            pos2 = j->data[i].first.header.find(delimiter2);
            readTxName = j->data[i].first.header.substr(pos1+1, pos2-pos1-1);
            //std::cout << "-----" << readTxName <<"--"<<pos1 <<"--"<< pos2 <<std::endl;
                ...
                if (!isPaired) {
                    ...
                    if (positionOK) {
                        if (compat) {
                            ...
                            //MAX
                            if (!readTxName.compare(transcripts[transcriptID].RefName)) {auxProbsCompat.push_back(1.0);} else {auxProbsCompat.push_back(0.0);}
                        ...
                        if (!haveCompat and !enforceCompat) {
                            ...
                            //MAX
                            if (!readTxName.compare(transcripts[transcriptID].RefName)) {auxProbsAll.push_back(1.0);} else {auxProbsAll.push_back(0.0);}
    ...
    ++hitIndex;
            ...
            if (haveCompat) {
                if (txpIDsCompat.size() > 0) {
                    ...
                    eqBuilder.addGroupTC(std::move(tg), auxProbsCompat);
                ...
                if (txpIDsAll.size() > 0) {
                    ...
                    eqBuilder.addGroupTC(std::move(tg), auxProbsAll);
    ...
    while(true) {
        ...
        for(size_t i = 0; i < j->nb_filled; ++i) { // For all the read in this batch
            ...
            if (jointHits.size() > 0) {
                ...
                //MAX - extract txname from read's header
                //std::cout << " \n header " << j->data[i].header<<std::endl;
                std::string delimiter1 = "/";
                std::string delimiter2 = " ";
                std::string readTxName;
                std::string::size_type pos1 = 0;
                std::string::size_type pos2 = 0;
                pos1 = j->data[i].header.find(delimiter1);
                pos2 = j->data[i].header.find(delimiter2);
                readTxName = j->data[i].header.substr(pos1+1, pos2-pos1-1);
...
int main(int argc, char* argv[]) {
    ...
    advanced.add_options()
        ...
        ("maxReadOcc,w", po::value<uint32_t>(&(sopt.maxReadOccs))->default_value(10000000), "Reads \"mapping\" to more than this many places won't be considered.")
...
*/
        ...
        bfs::path logPath = logDirectory / "MAX.log";
...
*/
        ...
        experiment.equivalenceClassBuilder().finishTC();
        ...
        bool expSuccess = feqExporter.writeFeq(experiment, sopt,feqOutputPath);//for MAX
        if (!expSuccess) {
        ...
        std::cerr << "For usage information, try " << argv[0] << " MAX --help\nExiting.\n";
        std::exit(1);
    }
    return 0;
}
